...
contract UserCreationContract {
    ...
    function userExists(string memory username) public view returns(Message memory) {
    ...
    function addToSignature(string memory userHash,string memory masterPublicKey,string memory signature) public returns(Message memory){
        Signature memory sign = Signature(userHash,masterPublicKey,signature,true);
    ...
    function validateUser(string memory username, string memory password) public view returns(Message memory) {
        address userAddress = usernameToAddress[username];
        User memory user = addressToUser[userAddress];
        Message memory message;
    ...
    function deleteUser(string memory password) public returns(Message memory){
    ...
    function changePass(string memory password) public returns(Message memory){
        address userAddress = msg.sender;
        User memory user = addressToUser[userAddress];
        Message memory message;
        if(keccak256(bytes(user.password)) == keccak256(bytes(password))){
            message = Message(500,"New password cannot be same as Old password");
            return message;
        }
